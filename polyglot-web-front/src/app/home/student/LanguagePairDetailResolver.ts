...
export class LanguagePairDetailResolver implements Resolve<boolean> {
  ...
  private loadingLessonHeaders = false;
  private loadingLatestTranslations = false;
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> {
    ...
    return merge(
      this.store
        .pipe(select(lessonHeaersLoadedForLanguagePair, {languagePairId}),
          tap(lessonHeadersLoaded => {
            if (!lessonHeadersLoaded && !this.loadingLessonHeaders) {
              this.loadingLessonHeaders = true;
              this.store.dispatch(StudentActions.loadLessonHeaders({languagePairId}));
            }
          }),
        ...
        .pipe(select(latestTranslationsLoadedForLanguagePair, {languagePairId}),
          tap(latestTranslationsLoaded => {
            if(!latestTranslationsLoaded && !this.loadingLatestTranslations){
              ...
              this.store.dispatch(StudentActions.loadLatestTranslations({languagePairId}))
            }
          }),
          filter(loaded => !!loaded),
          first(),
